...
import balefulmod.BalefulMod;
import balefulmod.util.CardStats;
import balefulmod.util.TriFunction;
...
import static balefulmod.util.GeneralUtils.removePrefix;
import static balefulmod.util.TextureLoader.getCardTextureString;


public abstract class BaseCard extends CustomCard {
    final private static Map<String, DynamicVariable> customVars = new HashMap<>();

    protected static String makeID(String name) { return BalefulMod.makeID(name); }
    ...
    public void upgrade()
    {
        if (!upgraded)
        {
            ...
            if (this.upgradesDescription)
            {
                ...
                {
                    BalefulMod.logger.error("Card " + cardID + " upgrades description and has null upgrade description.");
                }
                else
                {
