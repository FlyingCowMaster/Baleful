...
import balefulmod.util.GeneralUtils;
import balefulmod.util.KeywordInfo;
import balefulmod.util.Sounds;
import balefulmod.util.TextureLoader;
...
public class BalefulMod implements
    ...
    public void receiveEditCharacters() {
        Baleful.Meta.registerCharacter();
    }
    ...
    private static String checkResourcesPath() {
        String name = BalefulMod.class.getName(); //getPackage can be iffy with patching, so class name is used instead.
        ...
        if (!resources.exists()) {
            throw new RuntimeException("\n\tFailed to find resources folder; expected it to be at  \"resources/" + name + "\"." +
                    ...
                    "\t\"private static final String resourcesFolder = checkResourcesPath();\"\n" +
                    "\tat the top of the " + BalefulMod.class.getSimpleName() + " java file.");
        }
    ...
    private static void loadModInfo() {
        Optional<ModInfo> infos = Arrays.stream(Loader.MODINFOS).filter((modInfo)->{
            ...
            return initializers.contains(BalefulMod.class.getName());
        }).findFirst();
        if (infos.isPresent()) {
            info = infos.get();
            modID = info.ID;
